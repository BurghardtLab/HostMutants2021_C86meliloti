...
cbPalette <- c("#764e4e", "#ffca08", "#2998e3",  "#B23214","#7f723d", "#999999")
...
nod_counts <- read.table("../data/data_HostMutants2021_nod_counts.txt", sep="\t", header=TRUE)

nod_counts <- nod_counts %>% select(sample_id, batch, start, end, Slice, Count, total_area, average_size, percent_area, Circularity) %>% filter(sample_id!="833B")

all_data <- full_join(all_data, nod_counts, by="sample_id")
...
all_data <- all_data %>% inner_join(geno_name, by="genotype_id") %>%  mutate(cfu_plant = cfu_ml/4/num_plants_picked_nods, cfu_nods = log(cfu_ml/4/Count), above_plant = above_biomass_g/num_plants, below_plant = below_biomass_g/num_plants, shoot_root = above_plant/below_plant, nod_wt_plant = nodule_weight_g/num_plants_picked_nods, tot_biomass = (above_biomass_g + below_biomass_g)/num_plants, nod_plant=Count/num_plants_picked_nods, root_shoot=below_plant/above_plant)
...
#By host genotype
all_data_noko <- filter(all_data, type!="KO/OX")
p_cfu<-ggplot(all_data_noko, aes(x=reorder(name,order), y=cfu_plant, fill=type, na.rm=FALSE)) +
...
#filter out weird samples for now
zero <- c("858B","852E", "846A", "833A", "833C")
all_data_noko_zero <- filter(all_data_noko, !sample_id %in% zero)

p_cfu<-ggplot(filter(all_data_noko_zero, background=="A17"), aes(x=reorder(name,desc(order)), y=cfu_nods, fill=type, na.rm=FALSE)) +
        #scale_y_continuous(trans='log10') +
        labs(x="Plant Host",y="Population Size (log cfu/nod#)")+
        geom_boxplot() +
        geom_point() + 
        coord_flip() +
        scale_fill_manual(values = mycols$cols) +
...
#data_new <- all_data_noko_zero[(!is.numeric(all_data_noko_zero$cfus_nods)), ] 
#data_new <- all_data_noko_zero[(is.finite(all_data_noko_zero$cfus_nods)), ] 
#data_new <- data_new[!is.na(data_new$cfus_nods), ] 

#biomass_anova <- (lm(cfu_nods ~ name, data = filter(data_new, background=="A17")))
#summary(biomass_anova)


p_count<-ggplot(all_data_noko, aes(x=reorder(name,order), y=Count, fill=type, na.rm=FALSE)) +
        labs(x="Plant Genotype",y="Nod #")+
        geom_boxplot() +
        geom_point() + 
        coord_flip() +
        scale_fill_manual(values = mycols$cols) +
        theme_classic() + theme(legend.position="top", text = element_text(size=24))

p_count

p_area<-ggplot(all_data_noko, aes(x=reorder(name,desc(order), y=total_area, fill=type, na.rm=FALSE))) +
        #scale_y_continuous(trans='log10') +
        labs(x="Plant Genotype",y="Total Area nod (cm^2)")+
        geom_boxplot() +
        geom_point() + 
        coord_flip() +
        scale_fill_manual(values = mycols$cols) +
...
p_size<-ggplot(all_data_noko, aes(x=reorder(name,order), y=average_size, fill=type, na.rm=FALSE)) +
        #scale_y_continuous(trans='log10') +
        labs(x="Plant Genotype",y="average_size (cm^2)")+
        geom_boxplot() +
        geom_point() + 
        coord_flip() +
        scale_fill_manual(values = mycols$cols) +
...
p_<-ggplot(all_data_noko, aes(x=reorder(name,order), y=nod_plant, fill=type, na.rm=FALSE)) +
        #scale_y_continuous(trans='log10') +
        labs(x="Plant Genotype",y="nod per plant")+
        geom_boxplot() +
        geom_point() + 
        coord_flip() +
        scale_fill_manual(values = mycols$cols) +
...
ggsave(plot = p_cfu , width = 8, height = 9, dpi = 600, filename = "../figures/poster_cfu_nods_ordered.png")
...
#By host genotype
p_aBiomass<-ggplot(filter(all_data,background=="A17"), aes(x=reorder(name,desc(order)), y=above_plant, fill=type)) + 
...
ggsave(plot = p_aBiomass, width = 8, height = 9, dpi = 600, filename = "../figures/poster_above_biomass.png")
...
p_tot_biomass<-ggplot(filter(all_data_noko,background=="A17"), aes(x=reorder(name,desc(order)), y=tot_biomass, fill=type)) + 
        geom_boxplot(outlier.shape=NA) +
        geom_jitter() +
        coord_flip() +
        scale_fill_manual(values = mycols$cols) +
        labs(x="Plant Genotype",y="Total Biomass (g/plant)") +
        theme_classic() + theme(legend.position="", text = element_text(size=24))
p_tot_biomass 

p_bg_tot_biomass<-ggplot(all_data_noko, aes(x=background, y=tot_biomass)) + 
...
ggsave(plot = p_tot_biomass, width = 8, height = 9, dpi = 600, filename = "../figures/poster_total_biomass_a17.png")

biomass_anova <- (lm(tot_biomass ~ name, data = filter(all_data, background=="A17")))
summary(biomass_anova)
...
#By host genotype
p_shoot_root <-ggplot(filter(all_data_noko,background=="A17"), aes(x=reorder(name,desc(order)), y=shoot_root, fill=type)) + 
...
p_root_shoot <-ggplot(all_data_noko, aes(x=reorder(name,desc(order)), y=root_shoot, fill=type)) + 
        labs(x="Plant Genotype",y="Root:Shoot") +
        geom_boxplot() +
        geom_point() + 
        coord_flip() +
        scale_fill_manual(values = mycols$cols) +
...
p_bg_shoot_root<-ggplot(all_data_noko, aes(x=background, y=shoot_root)) + 
...
ggsave(plot = p_root_shoot , width = 8, height = 9, dpi = 600, filename = "../figures/poster_root_shoot_all.png")

biomass_anova <- (lm(root_shoot ~ name, data = filter(all_data, background=="A17")))
summary(biomass_anova)
...
#Lowest fitness values is Mag27 = -7.1 
...
rda.model.all<-anova(rda1, step=1000, perm.max=1000, by= "terms")  
...
#HEATMAP
#Fitness mean of all genotypes
fit.mean <- meta_fit %>% group_by(name) %>% summarise_if(is.numeric,mean)

fit.subtract <- column_to_rownames(fit.mean, "name")
fit.subtract <- fit.subtract[,-87]
pdf(file="../figures/poster_heatmap_base_mean_all.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.subtract), col=pal, trace="none", scale="row", cexCol = .5,cexRow = .75, offsetRow = -32)
dev.off()
...
# Run a permutational significance test for the explanatory variables in the full model
rda.model.all<-anova(rda2, step=1000, perm.max=1000, by= "terms") #
##write.table(rda.model.all,file = "../tables/RDA_r108.tsv",sep="\t",col.names = NA,row.names=TRUE)

#Plot the FULL model
smry <- summary(rda2)
#smry
df1  <- data.frame(smry$sites[,1:6])    
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA3,color=name)) + 
  geom_point()+
  scale_color_manual(values=cbbPalette)+
  geom_text(aes(label=name),size=8) +
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
  coord_fixed() +
  labs(x=paste("RDA 1 (",round(rda1_ev[1],2),"%)"),y=paste("RDA 3 (",round(rda1_ev[3],2),"%)"), color="Genotype") +
  theme_classic() + theme(legend.position="top", text = element_text(size=24)) 
rda.plot

ggsave("../figures/rda_r108_rda1-3.png", rda.plot, height = 6 , width = 8, dpi=600)
...
#fit.subtract <- bind_rows(fit.subtract[-8,], fit.mean[8,])
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]

pdf(file="../figures/heatmap_mean_r108.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row", cexCol = .5,cexRow = .75)
...
fit.mean.a17wt_dnf <- meta_fit_a17_dnf %>% filter(type=="WT") %>% group_by(name) %>% summarise_if(is.numeric,mean)

#Fitness mean of all genotypes in a17 host
fit.mean <- meta_fit_a17_dnf %>% group_by(name) %>% summarise_if(is.numeric,mean)

#Substract mean of wildtype from mean of mutant genotypes
fit.subtract<-as_tibble(cbind(fit.mean[,c(1:1)],mapply("-",  fit.mean[,c(-1,-88)], fit.mean.a17wt_dnf[,c(-1,-88)])))

fit.subtract <- bind_rows(fit.subtract[-1,], fit.mean[1,])
fit.subtract <- column_to_rownames(fit.subtract, "name")
fit.subtract <- fit.subtract[,-87]
#pdf(file="../figures/heatmap_adj_mean_a17_dnf_WT.pdf",width = 12,height=8)
...
fit.subtract <- column_to_rownames(fit.subtract, "name")
fit.subtract <- fit.subtract[,-87]
pdf(file="../figures/heatmap_adj_mean_a17_WT.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.subtract), col=pal, trace="none", scale="row", cexCol = .5,cexRow = .75)
dev.off()
...
# RDA analysis for fitness and plot RDA1 vs RDA2 and RDA3 vs RDA4
meta_fit_A18 <- meta_fit %>% filter(background=="NFS") %>% filter(type!="UN")
# Run a few different RDA Models
...
#copy sample_id(substring(right, 4)) in to a new column called sample_id
m_name <- select(filter(meta_fit,background=="NFS" & type!="UN"), name)
m_type <- select(filter(meta_fit,background=="NFS" & type!="UN"), type)
df1 <- df1 %>% add_column(m_name, m_type)
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA2, color=name)) + 
  scale_color_manual(values = cbbPalette) +
  geom_text(aes(label=name),size=9) +
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
  coord_fixed() +
  labs(x=paste("RDA 1 (",round(rda1_ev[1],2),"%)"),y=paste("RDA 2 (",round(rda1_ev[2],2),"%)")) +
  theme_classic() + theme(legend.position="top", text = element_text(size=24))
rda.plot
ggsave("../figures/rda_NILS.png", rda.plot, height = 6 , width = 8, dpi=600)


#HEATMAP
#Get mean of just NFS wildtype and subtract from mutants, then plot those values
#Fitness mean of NFS wildtype only
fit.mean.NFSwt <- meta_fit_A18 %>% filter(type=="WT") %>% group_by(name) %>% summarise_if(is.numeric,mean)
#Fitness mean of all genotypes in NFS host
fit.mean <- meta_fit_A18 %>% group_by(name) %>% summarise_if(is.numeric,mean)

#Substract mean of wildtype from mean of mutant genotypes
#fit.subtract<-as_tibble(cbind(fit.mean[,c(1:1)],mapply("-",  fit.mean[,c(-1,-88)], fit.mean.NFSwt[,c(-1,-88)])))

#fit.subtract <- bind_rows(fit.subtract[-8,], fit.mean[8,])
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]

pdf(file="../figures/heatmap_mean_NFS.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row", cexCol = .5,cexRow = .75)
...
freq<- freq %>% filter(type!="UN") %>% filter(background!="MS") %>% filter(type!="KO/OX")
...
anova(lm(shannon~name,data=filter(freq_div,background=="A17"))) # First pass anova shows 
summary(lm(shannon~name,data=filter(freq_div,background=="A17"))) # Examination of coeffecients suggests T
...
p<-ggplot(data=filter(freq_div,background=="A17"), aes(x=reorder(name, desc(order)), y=shannon, fill=type), na.rm=FALSE) +
  geom_boxplot() +
  geom_point() + 
  coord_flip() +
  scale_fill_manual(values=mycols$cols) +
...
ggsave("../figures/poster_shannon.png", p, height = 9 , width = 8, dpi=600)
...
p<-ggplot(data=freq_div, aes(x=reorder(name, order), y=invsimpson, fill=Type)) +
  ...
  theme_classic()
p
#ggsave("../figures/poster_invsimpson.png", p, height = 9 , width = 8, dpi=600)
```
...
```{r pie_chart}
sf <- data.frame(group=c( "WT-like","Decreased"), value=c(56, 44))
#value=c(61, 29))
bp<- ggplot(sf, aes(x="", y=value, fill=group))+
...
pie <- bp + coord_polar("y", start=-45) + scale_fill_manual(values=c("#764e4e", "#c59d00")) +
  blank_theme + 
  theme(axis.text.x=element_blank())+
    geom_text(aes(y = value/2 + c(0, cumsum(value)[-length(value)]), 
            label = paste(value,"%")), size=9)

pie

ggsave("../figures/poster_pie_r_sh.png", pie, height = 3 , width = 6, dpi=600)

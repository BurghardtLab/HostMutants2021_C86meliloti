...
split_data <- split(data_matrix, data_matrix$group_column)
...
split_data <- strsplit(as.character(df$x), ":")
split_data <- sapply(split_data, function(x) x[1])
split_data
library(ggplot2)

# Plotting all data points
p <- ggplot(summarized_rhizobia_data, aes(y = tajD, x = start)) +
  geom_point(color = cbPalette[3], alpha = 0.5) +  # Plot all data points with reduced opacity
  geom_smooth(method = "loess", span = 0.1, color = cbPalette[3]) +
  ggtitle("rhizobia Tajima's D") +
...
# Overlay points for specified locus tags
a<-p + geom_point(data = summarized_rhizobia_data[summarized_rhizobia_data$ncbi_locus_tag %in% split_data, ], 
               aes(y = tajD, x = start), 
               color = "red", 
               size = 1)
a

#plot them replicon wise.
```
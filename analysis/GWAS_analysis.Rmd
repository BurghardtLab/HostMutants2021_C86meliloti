...
```{r Code for SupplementalTableS8, echo=FALSE}
...
write.table(top5_candidates_crossranked.1,file="/Users/sohiniguha/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Sohini&Liana/GitHub/HostMutants2021_C86meliloti/tables/SupplementalTable S8.tsv",sep = "\t",row.names = FALSE)

```



```{r Code for Figure5 and Supplemental Figure4, echo=FALSE}
...
pdf(file = "/Users/sohiniguha/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Sohini&Liana/GitHub/HostMutants2021_C86meliloti/figures/Supplemental Figure4.pdf",width = 6.78, height = 6.78)
...
#beta heatmaps
pdf(file = "/Users/sohiniguha/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Sohini&Liana/GitHub/HostMutants2021_C86meliloti/figures/Supplemental Figure5.pdf",width = 6.78, height = 6.78)
...
pdf(file = "/Users/sohiniguha/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Sohini&Liana/GitHub/HostMutants2021_C86meliloti/figures/Supplemental Figure4.1.pdf",width = 6.78, height = 6.78)
...
pdf(file = "/Users/sohiniguha/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Sohini&Liana/GitHub/HostMutants2021_C86meliloti/figures/Supplemental Figure5.1.pdf",width = 6.78, height = 6.78)
...
df <- function(replicon) {
 ...
 #top5_candidates_crossranked1  <- top5_candidates_crossranked %>%separate(Location, c("Organism", "Replicon"))
  ...
  # Reciprocal transformation
  rownames(data_matrix_beta) <- result_beta$Beta
...
```{r CREATING the histograms of shared genomic basis, echo=FALSE}


crossranked_combined <- read.delim("~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Burghardt Lab Shared Folder/Projects/NSFPGRP_HostMutants/Data/GWAS_SOHINI/crossranked_combined.tsv")


crossranked_combined<-crossranked_combined%>% separate(Location,c('Organism','Replicon'))
crossranked_combined_psymA<-crossranked_combined[crossranked_combined$Replicon=="psyma",-c(2:7)]
within_rank10_counts_psyma <- rowSums(crossranked_combined_psymA[, -1] <= 10, na.rm = TRUE)


crossranked_combined_psymB<-crossranked_combined[crossranked_combined$Replicon=="psymb",-c(2:7)]
within_rank10_counts_psymb <- rowSums(crossranked_combined_psymB[, -1] <= 10, na.rm = TRUE)

crossranked_combined_chr<-crossranked_combined[crossranked_combined$Replicon=="chr",-c(2:7)]
within_rank10_counts_chr <- rowSums(crossranked_combined_chr[, -1] <= 10, na.rm = TRUE)

df.psyma<-data.frame(group=crossranked_combined_psymA$group,Occurence=within_rank10_counts_psyma)
df.psymb<-data.frame(group=crossranked_combined_psymB$group,Occurence=within_rank10_counts_psymb)
df.chr<-data.frame(group=crossranked_combined_chr$group,Occurence=within_rank10_counts_chr)
occurrence_counts.psyma <- table(df.psyma$Occurence)
occurrence_counts.psymb <- table(df.psymb$Occurence)
occurrence_counts.chr <- table(df.chr$Occurence)
p<-ggplot(df, aes(x=Occurence, color=Replicon,fill=Replicon)) +geom_histogram(position="dodge",bins=10)+ylab("Frequency")+theme_minimal()
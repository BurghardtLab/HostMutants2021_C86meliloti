...
#Table for Fig S6,S7
#write.table(combined_top10_candidates_data,file="../tables/SupplementalTable S7.tsv",sep = "\t",row.names = FALSE) 
...
```{r Code for ordering the beta heatmaps according to the prank heatmaps, echo=FALSE}
###ordering
# Function to generate FigS7 using the row order from FigS6
df <- function(replicon) {
  # Separate Location column into Organism and Replicon
 #top5_candidates_crossranked1  <- top5_candidates_crossranked %>%separate(Location, c("Organism", "Replicon"))
 ...
 result3<- result %>%
  ...
  # Define color palette
  pal1 <- colorRampPalette(c("lightgrey", "wheat", "gold", "orange", "darkorange", "red", "darkred"))(500)
  
  hv1 <- heatmap.2(data_matrix, Rowv = TRUE, Colv = TRUE, dendrogram = "row", col = pal1, 
                    hclustfun = function(d) hclust(d, method = "ward.D2"), trace = "none", 
                     scale = "none", cexCol = 1, cexRow = 0.5, key = TRUE, 
                   key.xlab = "Rank", key.ylab = NA, tracecol = "black", keysize = 2, 
                     symbreaks = FALSE, key.title = replicon, colCol = col_letters,
  ...
  # Extract the row order using hv1's rowInd
 
 ClusterOrder <- row.names(data_matrix[hv1$rowInd, ])
##creating the beta heatmaps
 #Remove Description and irrelevant columns for beta
 result_beta2 <- result_beta %>%
    select(-c(1:7)) %>%
    select(-Description)
  # Convert data for beta values to matrix
  data_matrix_beta <- as.matrix(result_beta2)
  
  #reorder matrix based on the row indices of the prank heatmap
   reordered_matrix <- data_matrix_beta[order(data_matrix_beta[,nrow(data_matrix)],),]
  
  # Reciprocal transformation
  rownames(reordered_matrix) <- result_beta$Description
...
pal = colorRampPalette(c('#2166AC',"#89ADD0", '#F7F7F7', '#B2182B'))
  # Generate heatmap
  heatmap.2(reordered_matrix, Rowv = FALSE,Colv=TRUE,dendrogram = "row", col = pal, 
            hclustfun = function(d) hclust(d, method = "ward.D2"), trace = "none", 
            scale = "none", cexCol = 1, cexRow = 0.5, key = TRUE, 
            key.xlab = "Beta", key.ylab = NA, tracecol = "black", keysize = 2, 
            symbreaks = FALSE, key.title = replicon, colCol = col_letters,
...
pdf(file = "../figures/FigS7_ordered.pdf",width = 6.78, height = 6.78)
df("chr")
df("psyma")
df("psymb")
dev.off()
...
```{r Code for creating FigS8, echo=FALSE}
pdf(file = "../figures/FigS8.pdf",width = 6.78, height = 6.78)
ggplot(top5_candidates_crossranked.1, aes(x=Rank_top10, color=Location, fill=Location)) +
    ...
    scale_fill_manual(values=c("black", "firebrick", "forestgreen")) +

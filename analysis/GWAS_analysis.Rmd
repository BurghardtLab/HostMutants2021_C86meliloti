...
combined_top10_data<-combined_top10_data%>%filter(!duplicated(ld_group))%>% select(-"replicon") 
...
#for Table S7
combined_top10_data<-combined_top10_data[combined_top10_data$p_lrt<= 1e-5,]%>%group_by(ld_group) %>%filter(!duplicated(ld_group))

write.table(combined_top10_data,file="/Users/sohiniguha/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Sohini&Liana/GitHub/HostMutants2021_C86meliloti/tables/SupplementalTable S7.tsv",sep = "\t",row.names = FALSE) 
...
top10<-rbind(combined_top10data_chr,combined_top10data_psyma,combined_top10data_psymb) #for the crossranked table

#select the top5
#make a list of top5,pr>=1 & pr<=5 (this is for the heatmaps)
top5_pr_candidates_psyma<-subset(combined_top10data_psyma,pr>=1 & pr<=5)
top5_pr_candidates_psymb<-subset(combined_top10data_psymb,pr>=1 & pr<=5)
top5_pr_candidates_chr<-subset(combined_top10data_chr,pr>=1 & pr<=5)
#combine the files
top5_pr<-rbind(top5_pr_candidates_psyma,top5_pr_candidates_psyma,top5_pr_candidates_psyma) #for the annotated heatmaps
#combined_top5<-combined_top5[combined_top5$p_lrt <= 1e-5,] #where do we use this?
...
write.table(combined_top5,file="/Users/sohiniguha/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Sohini&Liana/GitHub/HostMutants2021_C86meliloti/tables/top5candidates_condensed table.tsv",sep = "\t",row.names = FALSE)
```
...
#crossranked table:
top_candidates_crossranked<-data.frame(WTrank= merged_A17[match(unique(top10$ld_group),merged_A17$ld_group),]$pr,hclrank= merged_hcl[match(unique(top10$ld_group),merged_hcl$ld_group),]$pr,ipd3rank= merged_ipd3[match(unique(top10$ld_group),merged_ipd3$ld_group),]$pr,nad1rank= merged_nad1[match(unique(top10$ld_group),merged_nad1$ld_group),]$pr,dnf1rank= merged_dnf1[match(unique(top10$ld_group),merged_dnf1$ld_group),]$pr,dnf2rank=merged_dnf2[match(unique(top10$ld_group),merged_dnf2$ld_group),]$pr,dnf4rank=merged_dnf4[match(unique(top10$ld_group),merged_dnf4$ld_group),]$pr,dnf6rank=merged_dnf6[match(unique(top10$ld_group),merged_dnf6$ld_group),]$pr,dnf7rank=merged_dnf7[match(unique(top10$ld_group),merged_dnf7$ld_group),]$pr,dnf5dnf2rank=merged_dnf5dnf2[match(unique(top10$ld_group),merged_dnf5dnf2$ld_group),]$pr,dnf1dnf2rank=merged_dnf1dnf2[match(unique(top10$ld_group),merged_dnf1dnf2$ld_group),]$pr,rdn1rank=merged_rdn1[match(unique(top10$ld_group),merged_rdn1$ld_group),]$pr,sunn1rank=merged_sunn1[match(unique(top10$ld_group),merged_sunn1$ld_group),]$pr,sunn1rank=merged_sunn1[match(unique(top10$ld_group),merged_sunn1$ld_group),]$pr,sunn4rank=merged_sunn4[match(unique(top10$ld_group),merged_sunn4$ld_group),]$pr)

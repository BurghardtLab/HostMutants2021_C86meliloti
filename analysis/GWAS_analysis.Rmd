...
```{r Code for creating overlap, echo=FALSE}
...
overlap_all_fitness_shift <- lapply(cutoffs, get_overlap) %>% bind_rows()
overlap_all_fitness_shift

write.csv(overlap_all_fitness_shift, "../Tables/overlap_df_strainfitness_shift.csv", row.names = FALSE)
...
# -----------------------------
# 1️ Extract trait names from first row (columns 5:20)
trait_names <- as.character(data[1, 5:20])

# -----------------------------
# 2️ Remove first row (only contains trait names)
data_clean <- data[-1, ]

# -----------------------------
# 3 Convert trait columns to numeric
...
# -----------------------------
# 4️ Assign trait names as column names
colnames(data_clean)[5:20] <- trait_names

# -----------------------------
# 5️ Ensure Position is numeric
data_clean$Position <- as.numeric(as.character(data_clean$Position))

# -----------------------------
# 6 Map Chromosome based on SNP names
...
CMplot(cmdata,
       ...
       signal.cex=1,
       file="jpg",
       file.name="Strainfitness_shiftman",
       dpi=300,
       file.output=TRUE,
       verbose=TRUE,
...
# -----------------------------
# 1 Extract trait names from first row (columns 5:20)
trait_names <- as.character(data[1, 5:20])
...
# -----------------------------
# 3️ Convert trait columns to numeric
data_clean[, 5:20] <- lapply(data_clean[, 5:20], function(x) as.numeric(as.character(x)))
...
# -----------------------------
# 5 Ensure Position is numeric
data_clean$Position <- as.numeric(as.character(data_clean$Position))

# -----------------------------
# 6️ Map Chromosome based on SNP names
data_clean$Chromosome <- ifelse(
  grepl("chr", data_clean$SNP, ignore.case = TRUE), "chr",
  ifelse(grepl("psyma", data_clean$SNP, ignore.case = TRUE), "pSymA",
         ifelse(grepl("psymb", data_clean$SNP, ignore.case = TRUE), "pSymB", NA))

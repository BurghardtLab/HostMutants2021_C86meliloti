...
#Convert the coefficients into a  dataframe
  ...
  ##genotpye##
  myresults1<-data.frame(Response=c(paste(trait)),Coefficient=c(coefs.G$'Estimate'),Pvalue=c(coefs.G$'Pr(>|t|)'))
  myresults2<-data.frame(Genotype=c("(Intercept)","hcl","ipd3","nad1","dnf1","dnf2","dnf3","dnf4","dnf6","dnf7","dnf1/dnf2", "dnf5/dnf2","rdn1","sunn1","sunn4","repB","repC","repD","repE"))
  #Combine
...
```{r Write Function to extract Statistical summary of coefficients_attempt2, echo=TRUE}
...
#Convert the coefficients into a  dataframe
  coefs.G<-as.data.frame(coefs.G)
  coefs.T<-as.data.frame(coefs.T)
  # Create a data frame for genotype with the coefficients, trait, and p-value
myresults2<-data.frame(Response=c(paste(trait)),Coefficient=c(coefs.G$'Estimate',coefs.T$"Estimate"),Pvalue=c(coefs.G$'Pr(>|t|)',coefs.T$'Pr(>|t|)'))
#myresults2.T<-data.frame(Response=c(paste(trait)),Coefficient=coefs.T$'Estimate',Pvalue=coefs.T$'Pr(>|t|)')
#create a dataframe with the predictors
myresults1<-data.frame(Predictors=c("(Intercept)","hcl","ipd3","nad1","dnf1","dnf2","dnf3","dnf4","dnf6","dnf7","dnf1/dnf2", "dnf5/dnf2","rdn1","sunn1","sunn4","repB","repC","repD","repE","WT","Early","Late","AON","repB","repC","repD","repE"))
...
#how do i connect Classification with extract.summary????
}                  
...
TRAIT.summary<-rbind(extract.summary("nodules_perplant"),extract.summary("total_nodulated_area_perplant"),extract.summary("log_CFUpernodule"),extract.summary("log_CFUperplant"))
  
 #the code does not work for the following traits: total_area,total_biomass,ratio,average_size.

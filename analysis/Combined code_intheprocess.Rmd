...
Coef.Prop.Figure<-ggplot(data = Geno.Coef.Graphs) + 
  ...
  theme_bw()+
  coord_flip()+
  scale_fill_manual(values=c('#F7F7F7','#2166AC', '#B2182B','darkgoldenrod'))+xlab ("")+ylab("Proportion of Genotypes")+theme_bw()
...
Geno.Coef.Graphs$Class<-factor(Geno.Coef.Graphs$Class,levels=c("Down","None","Up","Change","Limited power"))
...
#Create your palette
pal = colorRampPalette(c('#2166AC', '#F7F7F7', '#B2182B','darkgoldenrod','#ccd6db'))(5)
...
heatmap.2(t(as.matrix(Geno_matrix)),Rowv=FALSE, Colv=FALSE,dendrogram = "none",col=pal, trace="none", scale="none", cexCol = .75,cexRow = .75, key=FALSE,srtCol=45, margins=c(5,8), lwid = c(1,15), lhei = c(1,15),na.color='#eaeef0',colCol = col_letters)
...
mean_fit<- rbind(meso_fit_med, inplanta_fit_med, longterm_FS)
...
M <- rcorr(as.matrix(fit_wide[,-1])) # correlation matrix # pvalue matrix
col1 = colorRampPalette(c("#AA1F2A", 'white', "#4696C5"))
pdf(file = "../figures/Fig4_fitnesscor.pdf",useDingbats = FALSE, height=6, width=6)
corrplot(M$r, method = "ellipse", type = "upper", diag = FALSE, order= "original", tl.col="black",addCoef.col = TRUE, col = col1(10), title = "Phenotypic Correlations", mar=c(0,0,2,0))
dev.off()

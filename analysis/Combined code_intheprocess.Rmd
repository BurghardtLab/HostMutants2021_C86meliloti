...
Geno.Coef.Graphs$Class<-factor(Geno.Coef.Graphs$Class,levels=c("Up","None","Down","Change"))
...
Coef.Prop.Figure<-ggplot(data = Geno.Coef.Graphs) + 
  ...
  theme_bw()+
  coord_flip()+
  scale_fill_manual(values=c("darkblue",'lightgrey',"darkred","darkgoldenrod"))+xlab ("")+ylab("Proportion of Genotypes")
...
heatmap.2(t(as.matrix(Geno_matrix)),Rowv=FALSE, Colv=FALSE, dendrogram = "none",col=pal, trace="none", scale="none", cexCol = .75,cexRow = .75, key=FALSE,srtCol=45, margins=c(5,8), lwid = c(1,15), lhei = c(1,15),na.color='lightgrey')
dev.off()

# colCol = col_letters
#adding the panel for relative strain fitness
#Selecting the data cols
#Geno.Coef.RDA<-Geno.Coef.RDA %>% select(c(Trait,Predictor,Class))
#Generating new rows
Newrows<-data.frame(Trait=c("nodule_isolate_fitness","nodule_isolate_fitness","nodule_isolate_fitness"),Predictor=c("dmi1-2","dmi2-3","dmi3-1"),Class=c("None","None","None"))
#Combine 
Geno.Coef.RDA<-rbind(Geno.Coef.RDA,Newrows)%>%column_to_rownames(var="Predictor")
# replace "None" values with 0,"Change" with "1"
Geno.Coef.RDA[is.na(Geno.Coef.RDA)|Geno.Coef.RDA=="Change"]<- 1
Geno.Coef.RDA[is.na(Geno.Coef.RDA)|Geno.Coef.RDA=="None"]<- 0
#Convert the Class col to numeric
Geno.Coef.RDA<-as.numeric(as.character(Geno.Coef.RDA))
#Convert it to a matrix
Geno.Coef.RDA.matrix<-as.matrix(Geno.Coef.RDA)
#define a color pallette
pal1 = colorRampPalette(c('#2166AC', '#B2182B'))(2)
#Construct the heatmap
heatmap.2(t(as.matrix(Geno.Coef.RDA.matrix)),Rowv=FALSE, Colv=FALSE, dendrogram = "none",trace="none",col=pal1, scale="none", cexCol = .75,cexRow = .75, key=FALSE,srtCol=45, margins=c(5,8), lwid = c(1,15), lhei = c(1,15))
#getting an error:x' must have at least 2 rows and 2 columns
```
...
col_letters = c("A17"="#E69F00","hcl"="#56B4E9","ipd3"="#56B4E9","latd"="#56B4E9", "nad1"="#009E73", "dnf1"="#009E73", "dnf2"="#009E73","dnf3"="#009E73", "dnf4"="#009E73","dnf6"="#009E73","dnf7"="#009E73", "dnf1/dnf2"="#009E73", "dnf5/dnf2"="#009E73", "rdn1"="#F0E442", "sunn1"="#F0E442","sunn4"="#F0E442")
...
heatmap.2(t(as.matrix(fit_mean_matrix)),Rowv=TRUE, Colv=TRUE,dendrogram = "both", col=pal,trace="none", scale="none",cexCol = 1,cexRow = .5, key=TRUE,key.xlab = "Mean Fitness", key.ylab =NA , tracecol="black",symbreaks=FALSE,key.title = NA,colCol = col_letters) #color key helps me understand the dynamics.

heatmap.2(t(as.matrix(fit_median_matrix)),Rowv=TRUE, Colv=TRUE, dendrogram = "both",col=pal, trace="none", scale="none", cexCol = 1,cexRow = .5, key=TRUE,key.xlab = "Median Fitness", key.ylab =NA , tracecol="black",symbreaks=FALSE,key.title = NA,colCol = col_letters)

myorder<-order(fit_mean_matrix["A17",])
heatmap.2(t(as.matrix(fit_mean_matrix[,myorder])),Rowv=FALSE, Colv=TRUE, dendrogram = "column",col=pal, trace="none", scale="none", cexCol = 1,cexRow = .5, key=TRUE,key.xlab = "Median Fitness", key.ylab =NA , tracecol="black",symbreaks=FALSE, key.title = NA,colCol = col_letters)
dev.off()

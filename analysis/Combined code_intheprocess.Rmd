...
Analysis file for NSF Host Mutant C86 Sinorhizobium meliloti select and resequence project.
...
```
```{r setup, include=FALSE}
# This file takes as input various files included in the Github repository including
...
```{r Strain Frequency: Import Data/Code for Supplemental Figure 1, echo=TRUE}
...
pdf(file = "../figures/Supplemental Fig1.pdf",width = 6, height = 6)
...
write.table (Geno.Coef.Summary,file="../tables/Supplemental Table S5.txt",sep = "\t",row.names = FALSE)
...
write.table (Type.Coef.Summary,file="../tables/Supplemental Table S3.txt",sep = "\t",row.names = FALSE)
```
...
write.table (Geno.Coef.RDA,file="../tables/Supplemental Table S6.txt",sep = "\t",row.names = FALSE)
write.table (Type.Coef.RDA,file="../tables/Supplemental Table S4.txt",sep = "\t",row.names = FALSE)
...
write.table (ANOVA.Trait.Summary,file="../tables/Supplemental Table S2.txt",sep = "\t",row.names = FALSE)

```

#Visualizing Results

```{r code for creating Supplemental Figure S2, echo=TRUE}
...
pdf(file = "../figures/Supplemental Figure2.pdf",width = 6, height = 6)
Coef.Prop.Figure
dev.off()
...
```{r code for creating Figure 2, echo=TRUE}

#Geno.Coef.Graphs$Class<-as.character(Geno.Coef.Graphs$Class)
#Geno.Coef.Graphs$Class[Geno.Coef.Graphs$Predictor =="latd"]<-"Limited power"
#Geno.Coef.Graphs$Class<-as.factor(Geno.Coef.Graphs)
...
pdf(file = "../figures/Figure2.pdf",width = 6, height = 3)
...
dev.off()
```
#heatmap.2(t(as.matrix(Geno_matrix)),Rowv=FALSE, Colv=FALSE, sepwidth=c(0.001,0.001),
           #sepcolor="black",
           #colsep=1:ncol(t(as.matrix(Geno_matrix))),
           #rowsep=1:nrow(t(as.matrix(Geno_matrix))),dendrogram = "none",col=pal, trace="none", scale="none", cexCol = .75,cexRow = .75, key=FALSE,srtCol=45, margins=c(5,8), lwid = c(1,15), lhei = c(1,15),na.color='lightgrey',colCol = col_letters)
#dev.off()


```{r code for creating Fig 3 and Fig 4, echo=TRUE}
...
pdf(file = "../figures/Fig3+Sup_Boxplots_SymbioticTraits.pdf",width = 6, height = 6)
...
trait.boxplot(trait = "cfu_per_plant_LOG10",trait_all = traits_all[traits_all$geno!="latd",])
...
```{r code for creating Figure 5, echo=TRUE}
...
pdf(file = "../figures/Figure 5.pdf",width = 5, height = 10)
...
```{r code for creating Figure S5-Phylogentic Heatmap, echo=TRUE}

library(ape)
## Read in the tree file created by Brendan
tree = read.tree('../data/parsnp_2020-12-26/rooted_by_figtree_2020-12-26.nw')

keep.tip(phy = tree,tip= tree$tip.label[!tree$tip.label %in% species$strain])
tree<-keep.tip(phy = tree,tip= paste(fitness$strain))
...
heatmap.phylo <- function(x, Rowp, Colp, ...) {
  ...
  image((1:ncol(x))-0.5, (1:nrow(x))-0.5, t(x), col=pal,
        xaxs="i", yaxs="i", axes=FALSE, xlab="",ylab="",breaks=seq(-9,5,0.1))
  par(mar=rep(0,4))
...
#### Double check this. might need slightly different structure ######
m = structure(as.matrix(fitness[, -1:-2]),
              dimnames=list(unlist(fitness[, 'strain']),
                            unlist(names(fitness)[-1:-2])))

col_dendro = as.dendrogram(hclust(dist(t(m))))

pdf(file="../figures/Mutant_Phylo_Heatmap_mean.pdf",width = 14,height=8, useDingbats=FALSE)
...
```{r RDA plots- Supplemental Figure S3}
...
pdf(file = "../figures/Supplemental Figure3.pdf",width = 5, height = 5)
...
ggplot(df1, aes(x=RDA3,y=RDA4,color=type))+
  ...
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
  labs(x=paste("RDA 1 (",round(rda1_ev[3],2),"%)"),y=paste("RDA 2 (",round(rda1_ev[4],2),"%)"), color="Type") +
  theme_classic() + theme(legend.position="top", text = element_text(size=12)) 
...
```{r cor plots}
# Correlation Plot
# Prep data for plotting combine soil and host fitness values only and turn into wide format
pdf(file = "../figures/Supplemental Figure_corr.pdf",width = 9, height = 9)

#merging the RDA analyses with the traits table
traits_all.1 <- left_join(traits_all, df1, by = "geno")
#retaining only the meaningful cols
traits_all.2<-traits_all.1%>% select(-sample_id,-rep,-genotype_id,-background,-date_biomass_harvested,-num_plants,-average_size,-percent_herbivory,-flowering,-num_plants_picked_nods,-type.y,-above_biomass_g,-below_biomass_g,-cfu_ml,-RDA3,-RDA4,-RDA5,-RDA6,-type.x,-Count,-total_area,-below_biomass_per_plant,-above_biomass_per_plant,-total_area)%>%group_by(geno)%>%summarise_if(is.numeric, mean, na.rm = TRUE)%>%filter(!geno %in% c("dmi1-2", "dmi2-3", "dmi3-1", "skl"))
...
corrplot.mixed(correlation_matrix, number.cex=0.6,upper = "ellipse", lower = "number",tl.pos = "lt", tl.col = "black",order = 'hclust', tl.offset=1, tl.srt = 90)
dev.off()
```

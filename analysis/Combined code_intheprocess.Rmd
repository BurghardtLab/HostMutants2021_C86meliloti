...
#p<-a.G$"Sum Sq"
#myresults<-data.frame(Response=c("Nod.Num"),Predictor=c("Genotype","Replicate","Residual"),Sum.Sq=p,PctExp=p/sum(p)*100)
#myresults
#q<-a.T$"Sum Sq"
#myresults<-data.frame(Response=c("Nod.Num"),Predictor=c("Genotype","Replicate","Residual","Type","Replicate","Residual"),Sum.Sq=c(p,q),PctExp=c(p/sum(p)*100,q/sum(q)*100))
#myresults
#write.table (myresults,file="../tables/SG_Variance_nodulenumber.txt",sep = "\t",row.names = FALSE)
```
...
write.table (ANOVA.Trait.Summary,file="../tables/Trait-Summary.txt",sep = "\t",row.names = FALSE)

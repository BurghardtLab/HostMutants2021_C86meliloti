...
pdf(file="../figures/htest_all.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```
...
rda.model.all<-anova(rda2, step=1000, perm.max=1000, by= "terms") # The interaction is significant, but temperature has the largest effect
...
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]
pdf(file="../figures/htest_r108.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```

```{r fit_rda_WT, echo=FALSE}
# RDA analysis for fitness and plot RDA1 vs RDA2 and RDA3 vs RDA4
meta_fit_WT <- meta_fit %>% filter(type=="WT") #| type=="NILS")  
# Run a few different RDA Models
rda1<-rda(meta_fit_WT[,c(2:87)]~name,meta_fit_WT, scale=TRUE) # Time only
rda2<-rda(meta_fit_WT[,c(2:87)]~background,meta_fit_WT, scale=TRUE) # Time only
rda3<-rda(meta_fit_WT[,c(2:87)]~background+name,meta_fit_WT, scale=TRUE) # Time only
#rda4<-rda(meta_fit_r108[,c(2:87)]~type+name,meta_fit_r108, scale=TRUE) # Time only
# Rsquared gives you an overall idea of well the overall model adjusted for the number of predictors you have

mymodels<-data.frame(data=c("WT"),model=c("name","background","background+name"),
...
rda.model.all<-anova(rda1, step=1000, perm.max=1000, by= "terms") # The interaction is significant, but temperature has the largest effect
write.table(rda.model.all,file = "../tables/RDA_WT.tsv",sep="\t",col.names = NA,row.names=TRUE)
...
#copy sample_id(substring(right, 4)) in to a new column called sample_id
m_name <- select(meta_fit_WT, name)
m_bckgd <- select(meta_fit_WT, background)
df1 <- df1 %>% add_column(m_name, m_bckgd)

rda1_ev <-as.vector(eigenvals(rda1, constrained=TRUE))

rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA2)) + 
  geom_point(aes(color=background, shape=name), size=4)+
  scale_shape_manual(values=c(15, 8, 10, 7,17,20,18,16,19))+
  scale_color_manual(values=mypal[c(4,6,8,10)])+
  geom_text(aes(label=name),size=4) +
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
  coord_fixed() +
  ...
  theme_classic() + theme(legend.position="top", text = element_text(size=14))
rda.plot
ggsave("../figures/rda_WT.png", rda.plot, height = 6 , width = 10, dpi=300)

fit.mean <- meta_fit_WT %>% group_by(name) %>% summarise_if(is.numeric,mean)
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]
pdf(file="../figures/htest_WT.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row", cexRow=0.75, cexCol=0.5)
dev.off()
```
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA2)) + 
  geom_point(aes(shape=name,color=type), size=4)+
  scale_shape_manual(values=c(15, 16, 10, 18,19,20,7,8,17))+
  scale_color_manual(values=mypal[c(4,6)])+
  geom_text(aes(label=name),size=4) +
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
...
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]
pdf(file="../figures/htest_a17_dnf.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA2)) + 
  ...
  scale_shape_manual(values=c(15, 16, 17, 18))+
  #scale_shape_manual(values=c(15, 16, 10, 18,19,20,7,8,17))+
  scale_color_manual(values=mypal[c(1,2,4,6)])+
  geom_text(aes(label=name),size=4) +
  geom_hline(yintercept=0, linetype="dotted") +
  geom_vline(xintercept=0, linetype="dotted") +
  coord_fixed() +
...
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]
pdf(file="../figures/htest_a17.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```
...
fit.mean <- meta_fit_MS %>% group_by(name) %>% summarise_if(is.numeric,mean)
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]
pdf(file="../figures/htest_MS.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```
...
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]
pdf(file="../figures/htest_NILS.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```
...
# RDA analysis for fitness and plot RDA1 vs RDA2 and RDA3 vs RDA4
meta_fit_ko <- meta_fit %>% filter(background=="R108") %>% filter(type=="KO/OX" | type=="WT")
# Run a few different RDA Models
...
#copy sample_id(substring(right, 4)) in to a new column called sample_id
m_name <- select(meta_fit_ko, name)
m_type <- select(meta_fit_ko, type)
df1 <- df1 %>% add_column(m_name, m_type)
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA3)) + 
  ...
  theme_classic() + theme(legend.position="top", text = element_text(size=14))
rda.plot
ggsave("../figures/rda_ko_rda1&3.png", rda.plot, height = 6 , width = 8, dpi=300)

fit.mean <- meta_fit_ko %>% group_by(name) %>% summarise_if(is.numeric,mean)
fit.mean <- column_to_rownames(fit.mean, "name")
fit.mean <- fit.mean[,-87]
pdf(file="../figures/htest_ko.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```
...
library("gplots")
library("RColorBrewer")
pdf(file="../figures/htest.pdf",width = 12,height=8)
h<-heatmap.2(as.matrix(fit.mean), col=pal, trace="none", scale="row")
dev.off()
```

...
# -----------------------------
cutoffs <- c(
  ceiling(0.001 * nrow(merged_A17)),  # 0.1%
  ceiling(0.01 * nrow(merged_A17)),   # 1%
  ceiling(0.1 * nrow(merged_A17))     # 10%
)
...
p<-ggplot(combined_overlap_df, aes(x = dataset, y = overlap_pct)) +
  ...
  geom_hline(
    data = combined_overlap_df %>%
      group_by(dataset, top_n) %>%
      summarise(median_obs = median(overlap_pct, na.rm = TRUE), .groups = "drop"),
    aes(yintercept = median_obs),
    linetype = "dashed",
    color = "black",
    linewidth = 0.8
  ) +
  facet_wrap(~ top_n, scales = "fixed", labeller = labeller(top_n = facet_labels)) +
  scale_color_manual(values = host_color_map) +
  scale_y_continuous(
    limits = c(0, 100),
    expand = expansion(mult = c(0.02, 0.05))
  ) +
  labs(
    y = "% overlap with A17 within top SNPs",
    x = NULL
  ) +
...
ggsave(
  filename = "../Figures/strain_overlap_plot.pdf",
  plot = p,
  width = 10,
  height = 6
)

...
# When the trait is just the fitness 
GWAS_traits_justfitness<-tibble(strains=row.names(t(fit_mean_matrix)),A17=x$A17,latd=x$latd,ipd3=x$ipd3,nad1=x$nad1,dnf2=x$dnf2,dnf3=x$dnf3,dnf6=x$dnf6,dnf7=x$dnf7,sunn4=x$sunn4,sunn1=x$sunn1,rdn1=x$rdn1,hcl=x$hcl,dnf1=x$dnf1,dnf4=x$dnf4,dnf5dnf2=x$`dnf5/dnf2`,dnf1dnf2=x$`dnf1/dnf2`)

write.table (GWAS_traits_justfitness,file="../tables/Mean_strainfitness.txt",sep = "\t",row.names = FALSE)
...
library(ggplot2)
library(dplyr)
library(ggsci)  # for publication-ready color palettes
...
pdf(file = "../figures/Pervasivegene_validationplots.pdf",width = 5, height = 5)



# Subset relevant columns
df2 <- df %>% dplyr::select(Host, Trt, LBMC, LBMC.gent.60., Percent.mutant.60.) 
...
df2$Percent.mutant.60. <- as.numeric(as.character(df2$Percent.mutant.60.))
df2$Percent.mutant.60.[is.na(df2$Percent.mutant.60.)] <- 0

# Create baseline data
baseline_value <- 0.76

# Filter relevant Hosts
freqs.sub <- df2 %>%
  filter(Host %in% c("A17", "dnf3", "dnf7", "sunn4"))

# Define host colors
host_colors <- c("A17" = "#1b9e77", "dnf3" = "#d95f02", "dnf7" = "#7570b3", "sunn4" = "#e7298a")

# Plot
p <- ggplot(freqs.sub, aes(x = Host, y = Percent.mutant.60., color = Host)) +
  # Boxplots
  geom_boxplot(outlier.shape = NA, width = 0.5, fatten = 1.2, color = "black", fill = NA) +
  # Jittered points
  geom_jitter(position = position_jitter(width = 0.15, height = 0), size = 2.5, alpha = 0.8) +
  # Single dashed line for baseline
  geom_hline(yintercept = baseline_value, linetype = "dashed", color = "grey40", size = 0.7) +
  # Annotate the dashed line
  annotate("text", x = 0.9, y = baseline_value + 0.02, label = "Initial Culture",
           color = "grey20", hjust = 0.4, fontface = "italic", size = 4) +
  # Color scale
  ...
  labs(
    x = "Plant Genotype",
    y = "Proportion of malonyl-CoA synthase mutant")

...
all_data <- all_data %>% inner_join(geno_name, by="genotype_id") %>%  mutate(cfu_plant = cfu_ml/4/num_plants_picked_nods, above_plant = above_biomass_g/num_plants, below_plant = below_biomass_g/num_plants, root_shoot = below_plant/above_plant)
...
p_bg_aBiomass<-ggplot(all_data, aes(x=background, y=above_biomass_g)) + 
        geom_boxplot(aes(x=background, y=above_biomass_g, fill=background)) +
...
p_bBiomass<-ggplot(all_data, aes(x=reorder(name,order), y=below_biomass_g)) + 
        ...
        theme_classic() + theme(legend.position="", text = element_text(size=16))
p_bBiomass 

p_bg_bBiomass<-ggplot(all_data, aes(x=background, y=below_biomass_g)) + 
        geom_boxplot(aes(x=background, y=below_biomass_g, fill=background)) +
...
```{r plot root_shoot, echo=FALSE}
#By host genotype
p_bBiomass<-ggplot(all_data, aes(x=reorder(name,order), y=root_shoot)) + 
        geom_boxplot(outlier.shape=NA) +
...
p_bg_bBiomass<-ggplot(all_data, aes(x=background, y=root_shoot)) + 
        geom_boxplot(aes(x=background, y=root_shoot, fill=background)) +

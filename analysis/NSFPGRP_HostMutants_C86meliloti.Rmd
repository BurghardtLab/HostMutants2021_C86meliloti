...
#mycols<-data.frame(Trt=c("FS","32C","4C","Na+","Dry","La","Sm","Clay"), cols=c("#666666","#D95F02", "#7570B3", "#E7298A", "#66A61E", "#E6AB02", "#A6761D", "#1B9E77"))
...
#By host genotype
p_aBiomass<-ggplot(all_data, aes(x=reorder(name,order), y=above_plant)) + 
        geom_boxplot(outlier.shape=NA) +
        geom_jitter(aes(x=name, y=above_plant, color=type)) +
        coord_flip() +
        scale_color_manual(values=cbPalette)  +
...
ggsave(plot = p_aBiomass, width = 10, height = 10, dpi = 600, filename = "../figures/above_biomass.png")
...
#By host genotype
p_shoot_root <-ggplot(all_data, aes(x=reorder(name,order), y=shoot_root)) + 
        geom_boxplot(outlier.shape=NA) +
        geom_jitter(aes(x=name, y=shoot_root, color=type)) +
        coord_flip() +
        scale_color_manual(values=cbPalette)  +
...
ggsave(plot = p_shoot_root , width = 10, height = 10, dpi = 600, filename = "../figures/shoot_root.png")
...
ggsave(plot = p_nod_wt , width = 5, height =5, dpi = 600, filename = "../figures/nod_wt_plant.png")
...
rda1<-rda(meta_fit[,c(2:87)]~background*type,meta_fit, scale=TRUE) #Full Model w/ interactions
rda2<-rda(meta_fit[,c(2:87)]~background+type,meta_fit, scale=TRUE) # Additive Model
rda3<-rda(meta_fit[,c(2:87)]~background,meta_fit, scale=TRUE) # Treatment only
rda4<-rda(meta_fit[,c(2:87)]~type,meta_fit, scale=TRUE) # Time only

# Rsquared gives you an overall idea of well the overall model adjusted for the number of predictors you have
mymodels<-data.frame(data=c("all"),model=c("host*type","host+type","host","type"),
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA2)) + 
  ...
  theme_classic()
rda.plot
ggsave("../figures/rda_plot.png", rda.plot, height = 15 , width = 10, dpi=300)
...
```{r fit_rda_r108, echo=FALSE, fig.width=8, fig.height=8}
# RDA analysis for fitness and plot RDA1 vs RDA2 and RDA3 vs RDA4
meta_fit_r108 <- meta_fit %>% filter(background=="R108") %>% filter(type!="UN")
# Run a few different RDA Models
...
#copy sample_id(substring(right, 4)) in to a new column called sample_id
m_name <- select(filter(meta_fit,background=="R108" & type!="UN"), name)
m_type <- select(filter(meta_fit,background=="R108" & type!="UN"), type)
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=PC1)) + 
  ...
  theme_classic()
rda.plot
ggsave("../figures/rda_r108.png", rda.plot, height = 15 , width = 10, dpi=300)
```

```{r fit_rda_a17, echo=FALSE, fig.width=8, fig.height=8}
# RDA analysis for fitness and plot RDA1 vs RDA2 and RDA3 vs RDA4
meta_fit_a17 <- meta_fit %>% filter(background=="A17")
# Run a few different RDA Models
rda1<-rda(meta_fit_a17[,c(2:87)]~type,meta_fit_a17, scale=TRUE) # Time only
rda2<-rda(meta_fit_a17[,c(2:87)]~name,meta_fit_a17, scale=TRUE) # Time only
...
# Rsquared gives you an overall idea of well the overall model adjusted for the number of predictors you have
mymodels<-data.frame(data=c("A17"),model=c("type"),
           Radj=c(RsquareAdj(rda1)$adj.r.squared),
           Prop.Exp=c(summary(rda1)$constr.chi/summary(rda1)$tot.chi))
mymodels
# The adjusted r-squared for the model allowing for an interaction between Time and Treatment has the highest rsquared even when penalized for the additional predictors
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA2)) + 
  ...
  theme_classic()
rda.plot
ggsave("../figures/rda_a17.png", rda.plot, height = 15 , width = 10, dpi=300)
```
...
```{r fit_rda_A18, echo=FALSE}
# RDA analysis for fitness and plot RDA1 vs RDA2 and RDA3 vs RDA4
meta_fit_A18 <- meta_fit %>% filter(background=="A18") %>% filter(type!="UN")
# Run a few different RDA Models
...
#copy sample_id(substring(right, 4)) in to a new column called sample_id
m_name <- select(filter(meta_fit,background=="A18" & type!="UN"), name)
m_type <- select(filter(meta_fit,background=="A18" & type!="UN"), type)
df1 <- df1 %>% add_column(m_name, m_type)
...
rda.plot <- ggplot(df1, aes(x=RDA1, y=RDA2)) + 
  ...
  theme_classic()
rda.plot
ggsave("../figures/rda_A18.png", rda.plot, height = 15 , width = 10, dpi=300)
```
...
p<-ggplot(data=freq_div, aes(x=Trt, y=shannon, fill=Type)) +
  ...
  theme_classic()
p
ggsave("../figures/shannon.png", p, height = 10 , width = 10, dpi=300)

...
res.aov <- aov(root_shoot ~ background, data = all_data) #filter(all_data, background=="A17"))
summary(res.aov)

res.aov <- aov(root_shoot ~ type, data = filter(all_data, background=="A17"))
summary(res.aov)
TukeyHSD(res.aov)

res.aov <- aov(root_shoot ~ type, data = filter(all_data, background=="R108"))
summary(res.aov)
...
ggsave(plot = p_bg_nod_wt , width = 5, height =5, dpi = 600, filename = "../figures/nod_wt_plant.png")
